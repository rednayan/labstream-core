...
//! two processes measure between their clocks. `lsl-time` already proves the
...
fn clock_sync_moves_a_timestamp_by_the_measured_offset() {
    ...
    inlet.set_postprocessing(lsl_time::flags::CLOCKSYNC);
    let got = inlet
...
fn a_deduced_timestamp_is_rebuilt_from_the_rate() {
    ...
    for k in 1..5 {
        outlet.push(&Sample {
            timestamp: lsl_wire::DEDUCED_TIMESTAMP,
