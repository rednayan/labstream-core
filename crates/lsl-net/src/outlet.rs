...
use lsl_proto::header::parse_block;
use lsl_proto::negotiate::{negotiate, FeedRequest, Outcome, ServerStream, LITTLE};
use lsl_proto::timesync;
use lsl_wire::{Codec, Sample, TEST_PATTERN_OFFSETS};
...
impl Outlet {
    ...
    pub fn new_blocking(info: StreamInfo) -> std::io::Result<Self> {
        if info.format == lsl_wire::Format::String {
...
fn serve_client(mut stream: TcpStream, shared: Arc<Shared>) -> std::io::Result<()> {
    ...
    for off in TEST_PATTERN_OFFSETS {
        let s = lsl_wire::test_pattern(info.format, info.channel_count as usize, off);
        codec
            .encode(&s, &mut out)
