...
use lsl_proto::discovery;
use lsl_proto::feed::{FeedClient, FeedParams, State, TestPatternError};
use lsl_time::{flags, PostProcessor};
use lsl_wire::Sample;
...
fn read_and_recover(
    ...
    let mut deducer = lsl_wire::TimestampDeducer::new(srate);
    ...
    loop {
        ...
        loop {
            match codec.decode(&carry) {
                ...
                Err(lsl_wire::Error::Truncated) => break,
                Err(_) => {
                ...
                match handshake(&fresh, Duration::from_secs(5), buffer_samples) {
                    Ok((s, c, rest)) => {
                        ...
                        if fresh.uid != known.uid {
                            ...
                            clock.mark_reset();
                            deducer = lsl_wire::TimestampDeducer::new(fresh.nominal_srate);
                        }
...
fn handshake(
    ...
    loop {
        match client.on_headers(&buf) {
            ...
            Err(lsl_proto::header::Error::Incomplete) => {}
