...
use labstream_wire::{ByteOrder, Codec, Format, Sample, TEST_PATTERN_OFFSETS};
...
impl FeedClient {
    ...
    pub fn on_test_pattern(&mut self, buf: &[u8]) -> Result<usize, TestPatternError> {
        ...
        for (i, pattern) in TEST_PATTERN_OFFSETS.iter().enumerate() {
            let (got, n) = match codec.decode(&buf[off..]) {
                Ok(x) => x,
                Err(labstream_wire::Error::Truncated) => return Err(TestPatternError::Incomplete),
                Err(e) => {
                    self.state = State::Failed;
                    return Err(TestPatternError::Decode(e));
                }
            };
            let want = labstream_wire::test_pattern(self.format, self.channels, *pattern);
    ...
    pub fn on_samples(&self, buf: &[u8]) -> Result<(Vec<Sample>, usize), labstream_wire::Error> {
...
mod tests {
    ...
    fn handshake(
        ...
        for off in TEST_PATTERN_OFFSETS {
            let s = labstream_wire::test_pattern(format, channels, off);
