...
//!
//! The same shape as `crates/labstream-wire/tests/mutants.rs`, applied to a state
//! machine instead of a codec.

use labstream_proto::header::{parse_block, Headers, StatusLine};
use labstream_proto::negotiate::{
...
fn every_mutant_is_caught_by_at_least_one_transcript() {
    ...
    // The dead header defect lives in the reader, so it needs its own pass.
    {
        ...
        for t in &all {
            ...
            let ours = answer_of(labstream_proto::negotiate(&req, &srv), &srv);
