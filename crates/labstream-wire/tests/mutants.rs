...
use labstream_wire::{ByteOrder, Codec, Format, Sample, Value};
...
fn correct_header(c: &Codec, s: &Sample, out: &mut Vec<u8>) {
    if s.timestamp == labstream_wire::DEDUCED_TIMESTAMP {
        out.push(1);
    } else {
        out.push(2);
...
const MUTANTS: &[Mutant] = &[
    ...
    Mutant {
        ...
        write: |c, s, out| {
            out.push(if s.timestamp == labstream_wire::DEDUCED_TIMESTAMP {
                1
            } else {
                2
            });
