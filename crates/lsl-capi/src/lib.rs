...
use lsl_net::{desc, ContinuousResolver, Inlet, Outlet, StreamInfo};
...
fn field_text(info: &StreamInfo) -> Vec<String> {
    vec![
        ...
        lsl_net::info::format_name(info.format).to_string(),
        info.source_id.clone(),
        lsl_net::info::show_point_16(info.nominal_srate),
        "1.100000000000000".to_string(),
        lsl_net::info::show_point_16(info.created_at),
        info.uid.clone(),
...
unsafe fn push_chunk_stamped(
    ...
    for k in 0..samples {
        ...
        let t = *stamps.add(k);
        b.outlet.push(&Sample {
            timestamp: if t == 0.0 { lsl_net::clock() } else { t },
            values,
        });
    }
    LSL_NO_ERROR
...
pub unsafe extern "C" fn lsl_resolve_byprop(
    ...
    let q = lsl_net::build_property_query(&session_id(), &cstr(prop), &cstr(value));
    let found = lsl_net::resolve(
        &q,
...
pub unsafe extern "C" fn lsl_create_continuous_resolver_byprop(
    ...
    let query = lsl_net::build_property_query(&session_id(), &cstr(prop), &cstr(value));
...
fn session_id() -> String {
    lsl_net::config::get().session_id.clone()
}
...
pub unsafe extern "C" fn lsl_get_fullinfo(
    ...
    let info = match lsl_net::read_fullinfo(&b.info, Duration::from_secs_f64(timeout.max(0.1))) {
        Ok(full) => full,
...
pub unsafe extern "C" fn lsl_set_config_filename(name: *const c_char) {
    lsl_net::config::set_filename(&cstr(name));
}
...
pub unsafe extern "C" fn lsl_set_config_content(content: *const c_char) {
    lsl_net::config::set_content(&cstr(content));
}
    ...
    if let Some(b) = (inlet as *mut InletBox).as_ref() {
        if let Some(i) = b.inlet.as_ref() {
            ...
            if !remote_time.is_null() {
                ...
                *remote_time = lsl_net::clock() - offset;
            }
            return offset;
        }
    }
